...
const Upload = () => { 
    ...
    const onSubmit = async (formValues) => { setLoading(true); 
    // first upload 
    ...
    const upload = await callAPI({ 
        ...
        const fileUrl = upload.data[0].url; const { title, caption } = formValues; 
        ...
        const submitPost = await callAPI({ url: "/posts", 
        method: "post", 
        data: payload, 
        headers: { 
            Authorization: `Bearer ${getJwt()}` } 
        ...
        <MainLayout> 
            <form onSubmit={handleSubmit}> 
                <div className="h-60 bg-gray-100 "> 
                    <label htmlFor="files" className=" w-full h-full cursor-pointer flex justi fy-center items-center "> 
                    {   
                        preview ? ( 
    ...
    <div className="p-3"> 
        <label htmlFor="title" className="block w-full mb-3"> 
            ...
            <input 
                ...
                { 
                                    ...
                                    <div className="text-xs text-red-500 pb-3">{errors.title}</div> 
                    ) 
                } 
            </label>
        </div> 
        <div className="p-3"> 
            ...
            { 
                                    ...
                                    <div className="text-xs text-red-500 pb-3">{errors.caption}</div> 
                        ) 
                    } 
                </label> 
            </div> 
